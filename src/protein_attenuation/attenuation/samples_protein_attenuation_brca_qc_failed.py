...
t_cors = read_csv('./tables/sample_attenuation_table.csv', index_col=0)
...
cors['attenuation_potential'] = [
    'Low' if i not in p_attenuated else ('High (> %.1f)' % (np.floor((t_cors.ix[i, 'diff'] if t_cors.ix[i, 'diff'] < .5 else .5) * 10) / 10)) for i in cors.index
]

# -- Plot
plot_df = cors.copy()
plot_df['attenuation_potential'] = [i if i == 'Low' else 'High' for i in plot_df['tumour']]
