...
# -- Gexp
transcriptomics = read_csv('%s/data/tcga_rnaseq.tsv' % wd, sep='\t', index_col=0).dropna()
...
# -- Regressions
genes = set(cnv.index).intersection(transcriptomics.index)
samples = set(cnv).intersection(transcriptomics)


# p = 'ERBB2'
def regressions(p, c):
    # Protein measurements
    y = transcriptomics.ix[p, samples].dropna()
    x = sm.add_constant(concat([cnv.ix[p], design], axis=True).ix[samples].dropna(), has_constant='add')
...
# -- Export
res.sort('fdr').to_csv('%s/tables/regressions_transcriptomics_cnv.csv' % wd, index=False)
...
plt.title('Transcriptomics ~ Copy Number')
sns.despine(trim=True)
plt.gcf().set_size_inches(3, 3)
plt.savefig('%s/reports/regressions_transcriptomics_cnv_qqplot.pdf' % wd, bbox_inches='tight')
plt.close('all')
