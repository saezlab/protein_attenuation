...
design = Series([samplesheet[i] for i in residuals], index=residuals.columns)
...
# -- Export
res.sort('fdr').to_csv('%s/tables/regressions_residuals_cnv.csv' % wd, index=False)
...
plt.savefig('%s/reports/regressions_residuals_cnv_qqplot.pdf' % wd, bbox_inches='tight')
plt.close('all')
