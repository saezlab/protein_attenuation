...
# -- TF activities
tf_activities = read_csv('%s/tables/tf_activities.csv' % wd, index_col=0)
print tf_activities
...
design = Series([samplesheet[i[:15]] for i in tf_activities], index=tf_activities.columns)
...
# --
genes = set(cnv.index).intersection(tf_activities.index)
samples = set(cnv).intersection(tf_activities)

y = cnv.ix[genes, samples].unstack()

x = tf_activities.ix[genes, samples].unstack()
x.name = 'gexp'
...
plt.savefig('%s/reports/pancan_tf_activities_cnv_aroc.pdf' % wd, bbox_inches='tight')
plt.close('all')
